...
<h2 name="filter">필터 (Filter)</h2>

필터란 무엇인가?
- 필터는 클라이언트가 어떤 서블릿에 대해 실행을 요청했을 때, 요청한 서블릿이 수행되기 전 혹은 서블릿 실행이 끝난 후 실행하게 하는 것이다. 
  쉽게 말해, 사용자의 요청을 가로채서 사전처리 or 사후처리 하는 것이다.
- 무조건 사전처리, 사후처리를 다 해야하는 건 아니고 둘 중 하나만 있어도 된다

메소드를 통해 호출할 수 있는데 왜 필터를 사용하나?
<b>여러개의 요청</b>을 한번에 가로챌 수 있기 때문이다. 다시 말해, 공통의 기능을 하나의 필터라는 기능에 모아두고 url로 걸러내어 필터 역할을 한다. 
예로, doFilter에 모아두면, 무조건 들르기만 하면 하나의 코딩으로 여러 기능을 구현할 수 있다. 


모든 패턴을 url패턴에 들어가게만 해주면 된다!
